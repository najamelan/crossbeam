...
impl <T> Sender<T> {
    ...
    pub fn poll_ready(&mut self) -> Poll<(), SendError<()>> {
        ...
        // If channel is bounded, the option is a usize, so the unwrap is safe.
        //
        if self.channel.items.len() >= self.channel.items.capacity().unwrap() {
            ...
            let task = match task {
                ...
                None => {
                    ...
                    self.channel.sender_tasks.send(task.clone(), None).map_err(|_| SendError(()))?;
                    self.channel.receiver_task.notify();
                    task
                }
            };
